...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Guruacharya Timilsina app for verifying linear stability of Lokta Voltera Equations"),
  #ITS SOLUTION HAS ONLY H COMPONENT AND NO P COMPONENT. OUTPUT IS JUSTIFIED. 
  ...
  myOutput <- reactive({
    ...
    Lorenz<-function(t, state, parameters) {
      with(as.list(c(state, parameters)),{
        # rate of change
        dH <- -input$beta*input$gamma/input$delta*P
        dP <- -input$alpha*input$delta/input$beta*P
        #    dZ <- b*X*Y + X*Z - Z     
